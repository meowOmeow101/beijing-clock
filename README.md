...
| 从最近任务隐藏 | 开启后应用不出现在最近任务里，没有「划掉后台」这个动作，通知栏时间最不容易中断 |
...
界面上的「从最近任务隐藏」是这套保活里最管用的一招：应用不出现在最近任务列表里，
用户就没有「划掉后台」这个动作，前台服务不会被打断。它用
`FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS` 实现，而不是清单里的 `excludeFromRecents="true"`，
也不是禁用 Activity 组件 —— 前者无法运行时开关，后者会让通知栏那条通知也点不开应用
（Android 11 起被禁用的组件连显式 Intent 都会被拦下）。代价是通知栏成为进入应用的唯一入口，
所以只有通知栏时间处于开启状态时才允许打开这个开关。

服务内部为这件事做了几处防护：`onStartCommand` 兜住 `startForeground` 可能抛出的异常，
避免重建时陷入崩溃循环；WakeLock 带 12 小时超时申请、并在 10 小时处自动续期，
避免连续运行超过半天后息屏秒数停住；`onTaskRemoved` 里立刻重启服务（并补两次重试），
因为 Android 12 起给的这个「允许从后台启动前台服务」窗口只有几秒。
...
当前 58 项检查全部通过，覆盖：向真实 NTP 服务器取时并把结果和 UTC+8 格式化对照、
首选服务器失败时自动切换备用、偏移量计算与 `now()` 一致、每次打开都会重新校准、
校准进行中重复触发被去重、进程重启后偏移量仍然生效、系统时钟被回拨时丢弃旧偏移量，
以及保活规则的判定（退出后是否重启服务、是否隐藏于最近任务、WakeLock 何时该续期、
界面状态文案、进程重建诊断）。
